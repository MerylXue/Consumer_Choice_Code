...
class EC_bench(ConsumerChoice):
    def __init__(self, n, num_assor, Single_Assor_Data, lb, ub, n_samples):
        super(EC_bench, self).__init__(n, num_assor, Single_Assor_Data, lb, ub, n_samples)
        self.lambda_ = np.random.uniform(0, 1)
        ...
        for i in range(self.T_test):
            ...
            for j in range(num_choice):
                ...
                for m in np.arange(j, num_choice):
                    sum_u += self.utility[rank_test[i][m]]-self.utility[rank_test[i][j]]   
                G_test[i][rank_test[i][j]] = math.exp(- self.lambda_ * sum_u)/(num_choice-j) #j in 0-10, n=10
